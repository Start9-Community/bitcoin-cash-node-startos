...
- **Tor is SOCKS-only.** Outbound peers route through tor's SOCKS proxy via `sdk.getContainerIp(effects, { packageId: 'tor' })` + `:9050`; inbound onion uses the Tor service's URL plugin on the Peer interface (`-listenonion` is force-disabled — tor-startos has no TCP control port).
